...
- The apex TXT record on `forestrie.dev` (`v=MCPv1; k=ed25519; p=<pubkey>`,
  generated by `mcp-publisher login dns`) must exist —
...
Without either, a tag push runs the npm publish and then fails at the
`mcp-publisher login dns` step — the npm release is unaffected;
...
- Registration fails without them; the third variable the
